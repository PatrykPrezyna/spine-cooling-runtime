...
class ThermistorHardwareTests(unittest.TestCase):
    def test_labels_include_thermistors(self) -> None:
        names = temperature_labels_from_config(_CONFIG)
        self.assertEqual(names, ["CSF", "Therm 1", "Therm 2"])

    def test_label_helpers_split_by_sensor_family(self) -> None:
        self.assertEqual(thermocouple_labels_from_config(_CONFIG), ["CSF"])
        self.assertEqual(
            thermistor_labels_from_config(_CONFIG), ["Therm 1", "Therm 2"]
        )

    def test_sim_reads_thermistors_and_pressure(self) -> None:

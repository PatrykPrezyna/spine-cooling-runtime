...
from spine_cooling.sensor_manager import SensorManager
...
def run_application(config_path: Path, daemonize: bool) -> int:
...
def main() -> int:
    ...
    if args.daemon:
        try:
            ...
            with DaemonContext():
                return run_application(Path(args.config), daemonize=True)
        except ImportError:
            print("The python-daemon package is required for daemon mode.")
            return 1
    return run_application(Path(args.config), daemonize=False)
